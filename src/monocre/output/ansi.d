...
void format256Color(Sink sink, CharImage.Color color, bool fg)
{
	static immutable ansiPal = [
 	...
 	].map!(CharImage.Color.fromHex).array;

	static int colorDist(CharImage.Color a, CharImage.Color b)
	{
		// Naive weighted Euclidean distance
	...
	auto index = iota(16, ansiPal.length)
		.map!(i => tuple(colorDist(ansiPal[i], color), i))
		.reduce!min
		[1];
